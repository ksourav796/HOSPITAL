 ...
 *
 */
/* add code for responsive popup  for small device and tablet by @ambi  */ 
 var x1, y1, unq, cou = 0, cou1, Act_wth = [], A_wth = [], Act_id = [] , A_hei = [], Act_hei = []; 
/* End code for responsive popup  for small device and tablet by @ambi */
(function($) {
...
/* add code for responsive popup  for small device and tablet by @ambi  */
        if (Act_wth[cou] === undefined && opts.width !== undefined && $.data(_221, "panel").panel.find(">div.panel-body").attr('id') !== undefined) {
            if (cou === 0) {
                Act_wth[cou] = opts.width;
                if(opts.height !== undefined){
                  Act_hei[cou] = opts.height;
                }
                Act_id[cou] = $.data(_221, "panel").panel.find(">div.panel-body").attr('id');
                cou1 = cou;
                cou++;
            } else {
                if ($.data(_221, "panel").panel.find(">div.panel-body").attr('id') !== Act_id[cou1]) {
                    if (!(unique($.data(_221, "panel").panel.find(">div.panel-body").attr('id')))) {
                        Act_wth[cou] = opts.width;
                        if(opts.height !== undefined){
                            Act_hei[cou] = opts.height;
                        }
        ...
        for (x1 = cou; x1 >= 1; x1--) {
            if (_223.children("div.panel-body").attr('id') === Act_id[x1]) {
                A_wth[x1] = Act_wth[x1];
                A_hei[x1] = Act_hei[x1];
                y1 = x1;
            }
        ...
        if (opts.top !== null) {
           if((w >= 993 && w <= 1150)){
              if(759 < A_wth[y1]){                              
                 _223.children('.easyui-dialog').panel('resize',{
                     width: 880,
                     height: 'auto',
                     maxHeight:500,
                     left: (w-880)/2                                    
                  });
                 _223.children('.panel-header').css('width','870'); 
                 _223.children('.easyui-dialog').css('width','870'); 
               }else{ 
                  _223.children('.easyui-dialog').panel('resize',{
                     width: A_wth[y1],
                     height: A_hei[y1],  
              ...
              if(759 < A_wth[y1]){
                 ...
                 _223.children('.panel-header').css('width','750'); 
                 _223.children('.easyui-dialog').css('width','750'); 
              } 
              else{  
                  _223.children('.easyui-dialog').panel('resize',{
                     width: A_wth[y1],
