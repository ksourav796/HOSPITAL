...
app.controller('loginController',
    function($scope, $http, $location, $filter, Notification, ngTableParams,  $timeout, $window, $rootScope){
        ...
        $scope.login = function () {
            ...
            $http.post("hospital/login", JSON.stringify(loginData)).then(function success(response) {
        ...
        $scope.saveuservalidate = function () {
            ...
            saveUserDetails = {
                ...
                password: $scope.pswdText,
            };

            $http.post("/hospital/userValidate", angular.toJson(saveUserDetails)).then(function (response) {
                ...
                if(data==""){
                    ...
                    $window.location.href = '/home#!/calender';
                    $('#sidebar-menu').show();
                    $('#logoutButton').show();
                    $scope.removeuserDetails();
        ...
        $scope.saveuserDetails = function () {
            if ($scope.userText === '') {
                ...
                $http.post("/hospital/saveLoginDetails", angular.toJson(saveUserDetails)).then(function (response) {
                    var data = response.data;
                    if (data == "") {
